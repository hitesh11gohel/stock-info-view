...
const GridData = ({ item, live }) => {
  ...
  return (
    <Box
      ...
      component={Paper}
      sx={{ width: "600px", maxWidth: "100vw" }}
    >
      <Grid container direction="row" alignItems="center" className="">
        <Grid item xs={12} sm={12} md={12}>
          <h5 className="no-margin fw-bold card-title">
            {code}{" "}
      ...
      <Grid container direction="row" alignItems="center" className="fw-bold">
        <Grid item xs={6} sm={6} md={6} />
        <Grid item xs={3} sm={3} md={3} className="text-end">
          <p className="no-margin mkt-price">
      ...
      <Grid container direction="row" alignItems="center" className="mt-2">
        ...
        <Grid item xs={3} sm={3} md={3} className="text-end">
          <small className="no-margin" style={{ fontSize: "11px" }}>
            52 weeks
          </small>
        </Grid>
        <Grid item xs={3} sm={3} md={3} className="text-end ">
        ...
        <Grid item xs={3} sm={3} md={3} className="text-end">
          <div className="d-flex justify-content-between">
            <p className="no-margin slash" style={{ marginLeft: "5px" }}>
              /
            </p>
      ...
      >
        <Grid item xs={4} sm={4} md={4}>
          ...
          <h6 className="text-success no-margin card-gain-price">
            + 4.95 (0.50%)
          </h6>
        </Grid>
        <Grid item xs={6} sm={4} md={4} className="text-end">
          ...
          <h6
            className={
              color === "green"
                ? "text-success no-margin card-gain-price"
                : "text-danger no-margin card-gain-price"
            }
          >
            {totalGain
              ? profit > 0
                ? `+ ${profit.toFixed(2)} (${totalGain.toFixed(2)}%)`
                : `${profit.toFixed(2)} (${totalGain.toFixed(2)}%)`
              : profit}
          </h6>
        </Grid>
  ...
  return (
    <>
      <div className="d-flex justify-content-center">
        <Link
          key={liveData._id}
          to={`/${propData.code}/${liveData._id}`}
          className="text-decoration-none"
          onClick={() => props.setSingleStockDetail(propData)}
        >
